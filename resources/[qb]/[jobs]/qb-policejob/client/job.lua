    ...
    if coords then
        ...
        QBCore.Functions.TriggerCallback('police:server:PayForVehicle', function(result)
            ...
            QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
                ...
                TriggerServerEvent('police:server:addTrunkItems', plate, Config.CarItems)
                SetVehicleEngineOn(veh, true, true)
...
RegisterNetEvent("police:client:ImpoundMenuHeader", function (data)
    if not IsPedInAnyVehicle(PlayerPedId(), false) then
        ...
        QBCore.Functions.Notify(Lang:t("error.while_incar"), "error")
    end
end)
...
RegisterNetEvent("qb-police:client:openStash", function()
    if Config.Inventory == 'ox_inventory' then
        ...
        local name = 'Police Locker '..citizenid
...
RegisterNetEvent('qb-police:client:openTrash', function(k)
    if Config.Inventory == 'ps-inventory' or Config.Inventory == 'qb-inventory' then
...
RegisterNetEvent('police:client:TowMenu', function(Vehicle, Plate)
    ...
    TowMenu[#TowMenu+1] = {opthead = Lang:t('menu.close_x'), opticon = 'xmark', optparams = { event = '', args = {}}}
    local header = {
        disabled = true,
        header = Lang:t('info.tow_vehicle'),
